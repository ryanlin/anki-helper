...
async def add(ctx):
    ...
    try: 
        ...
        result, llm_response = create.generate_flashcard(query)
        success = result['result'] != None
        if success:
            await ctx.send('Done! Added example **(' + llm_response['fields']['Simplified'] + ')**: ' + llm_response['fields']['SentenceSimplified'])
        else:
            raise result['error']
